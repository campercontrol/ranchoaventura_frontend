...
@Component({
  ...
  styleUrls: ['./admin-edit-staff.component.scss']
})
export class AdminEditStaffComponent implements OnInit {
  visibleSelection = 0;
  blood_types: any = [];
  ...
  getPerfil(){
    ...
    this.staff.infoPerfil(this.updateId).subscribe((res:any)=>{
      console.log(res);
      this.vaccines = res.vaccines;
      this.food_restrictions = res.food_restrictions;
      this.blood_types = res.blood_types;
      let staff = res.staff
        this.photoSelect = 'https://api.kincamp.com/'+staff.photo;
