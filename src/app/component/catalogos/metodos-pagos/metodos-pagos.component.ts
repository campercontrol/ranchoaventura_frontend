...
export class MetodosPagosComponent implements OnInit {
  ...
  getCatalogos() {
    this.catalogos.getpayment_method().subscribe((res: any) => {
  ...
  guardar() {
    this.catalogos.postpayment_method(this.formFood.value).subscribe((res: any) => {
      this.getCatalogos();
  ...
  keepUpdate(){
    this.catalogos.upddatepayment_method(this.formFood.value,this.updateId).subscribe((res: any) => {
     console.log(res);
