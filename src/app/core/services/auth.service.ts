...
export class AuthenticationService {
    ...
    return this.http.post("https://api.kincamp.com//token?username=" +email+ "&password="+password +"&lang=es",{ })
    }
    login2(email: string, password: string) {
      return new Promise((resolve,reject)=>{
           this.http.post("https://api.kincamp.com//token?username=" +email+ "&password="+password +"&lang=es",{ })
          .subscribe((user:any) => {
              console.log(user);
              this.loggedIn = true;
